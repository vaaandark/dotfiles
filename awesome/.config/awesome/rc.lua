...
-- Awesomewm exit, inspired by i3exit
local exit = require("exit")

-- Status bar
local bar = require("statusbar")
local separator = bar.separator

-- For debug
local function printf(fmt, ...)
  printf(string.format(fmt, table.unpack(arg)))
end

-- Notification wrapper to set default icon
local notify_ = naughty.notify
naughty.notify = function(args)
  local path
  args = args or {}
  local preset = args.preset or {}
  -- Just a temporary workaround
  -- `if preset == naughty.config.presets.critical` doesn't work
  -- because Lua always compares tables' addresss
  if preset and preset.timeout == 0 then -- Urgent Notification
    -- Urgent notification should be detailed
    path = os.getenv("HOME") .. "/.config/awesome/images/critical.png"
  else
    path = os.getenv("HOME") .. "/.config/awesome/images/normal.png"
    -- Normal notification can be partly omitted
    args.max_height = 200
    args.max_width = 600
  end
  preset.icon = preset.icon or path
  args.preset = preset
  if args.title then
...
-- Decide the size of popup client by appoint its width / height
local function popup_default_size()
  local s = awful.screen.focused().geometry
  local h, w = s.height, s.width
  -- If the width-height ratio is more than 16 / 9 ?
  if 9 * w > 16 * h then
    -- Prefer width
    return "width", w * 0.6
  else
...
terminal = "alacritty"
floating_terminal = "alacritty --class=floating-term"
...
mylauncher = awful.widget.launcher({ image = beautiful.release_icon, menu = mymainmenu })
...
local taglist_buttons = gears.table.join(
                      awful.button({ }, 1, function(t)
                                             local now_screen = awful.screen.focused()
                                             if t == now_screen.selected_tag then
...
local function reset_apps_dpi(s)
  -- Wallpaper
...
globalkeys = gears.table.join(
  ...
  awful.key({ modkey, "Shift" }, "Return", function()
            ...
            { description = "open a float terminal", group = "externals" }),
  ...
  awful.key({ modkey }, "w", function()
                               ...
                               local now_layout = now_tag.layout
                               if now_layout ==  awful.layout.suit.max then
                                 if now_tag.last_layout then
                                   awful.layout.set(now_tag.last_layout)
                                   now_tag.last_layout = nil
                                 end
                               else
                                 now_tag.last_layout = now_layout
                                 awful.layout.set(awful.layout.suit.max)
                               end
                             end,
            { description = "set the layout of current tag max", group = "layout" }),
  awful.key({ modkey }, "e", function()
                               local now_tag = awful.screen.focused().selected_tag
                               local now_layout = now_tag.layout
                               if now_layout ==  awful.layout.suit.spiral then
                                 if now_tag.last_layout then
                                   awful.layout.set(now_tag.last_layout)
                                   now_tag.last_layout = nil
                                 end
            ...
            { description = "set the layout of current tag spiral", group = "layout" }),
...
for i = 1, 9 do
  globalkeys = gears.table.join(globalkeys, -- View tag only.
    awful.key({ modkey }, "#" .. i + 9,
              function()
                local now_screen = awful.screen.focused()
                local tag = now_screen.tags[i]
                if tag then
                  if tag == now_screen.selected_tag then
                    awful.tag.history.restore()
                  else
                    tag:view_only()
                  end
...
for _, i in ipairs(externals.keybindings) do
  local mod, key, command, discription = table.unpack(i)
  globalkeys = gears.table.join(
                 globalkeys,
                 awful.key(mod, key, function ()
                                       awful.util.spawn(command)
                                     end,
                           { discription = discription, group = "externals" }
                 )
  )
end
...
function restore_windows_to_desired_screen()
...
screen.connect_signal("request::desktop_decoration", function(s)
  ...
  restore_windows_to_desired_screen()
end)
-- }}}
